...
Tests BM25+FAISS search quality against two sources:
  - GOLD: manually curated query set (always present)
  - synthetic_gold.json: LLM-generated questions (loaded if file exists)

Generate synthetic_gold.json once with:
    cd backend
...
# ---------------------------------------------------------------------------
# Load synthetic questions if available (generated by scripts/generate_synthetic_questions.py)
# ---------------------------------------------------------------------------

_SYNTHETIC_PATH = Path(__file__).parent / "synthetic_gold.json"

def _load_synthetic() -> list[tuple[str, str, set[str]]]:
    if not _SYNTHETIC_PATH.exists():
        return []
    try:
        items = json.loads(_SYNTHETIC_PATH.read_text(encoding="utf-8"))
        return [
            (item["query"], item["language"], set(item["expected_doc_ids"]))
            for item in items
            if item.get("query") and item.get("language") and item.get("expected_doc_ids")
        ]
    except Exception:
        return []
...
# Combined set used for the aggregate evaluate_all() report
ALL_GOLD = GOLD + SYNTHETIC
...
@pytest.mark.skipif(not _SYNTHETIC_PATH.exists(), reason="synthetic_gold.json not generated yet")
@pytest.mark.parametrize("query,language,expected_ids", SYNTHETIC)
def test_retrieval_hits_synthetic(query: str, language: str, expected_ids: set[str]) -> None:
    """At least one expected doc must appear in top-K for synthetic queries."""
    results = _get_results(query, language)
    hit = bool(set(results) & expected_ids)
    assert hit, (
        f"Synthetic query: {query!r} | expected any of {expected_ids} in top-{K}, "
        f"got: {results}"
    )
...
def evaluate_all() -> dict[str, Any]:
    ...
    dataset = ALL_GOLD
    label = f"GOLD ({len(GOLD)}) + synthetic ({len(SYNTHETIC)})" if SYNTHETIC else f"GOLD ({len(GOLD)})"

    print(f"\nDataset: {label}")
    print(f"{'Query':<45} {'Lang':<5} {'Expected':<20} {'Got':<30} P@{K}  R@{K}  RR")
    print("-" * 120)

    for query, language, expected_ids in dataset:
        results = _get_results(query, language)
    ...
    return {
        ...
        "hit_rate": round(hits / len(dataset), 3),
    }
